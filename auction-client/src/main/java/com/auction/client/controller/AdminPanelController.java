...
public class AdminPanelController extends BaseController {
    ...
    public void refreshAdminDataFromServer() {
        ...
        try {
            // Gửi 2 request song song — ServerListener merge khi cả 2 về đủ
            NetworkService.getInstance().sendNetworkMessage(
                    new NetworkMessage(MessageType.GET_ALL_AUCTIONS_REQUEST, "{}"));
            NetworkService.getInstance().sendNetworkMessage(
                    new NetworkMessage(MessageType.GET_ALL_USERS_REQUEST, "{}"));
        } catch (Exception e) {
